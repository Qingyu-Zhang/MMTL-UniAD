...
from Fusionlist.AFF_fusion_ori import AFF 
from Fusionlist.AFF_fusion_ori import ECAAttention 
from attention.biformer import BiLevelRoutingAttention
...
class TotalNet(nn.Module):
    def __init__(self):
        ...
        # Adding BiLevelRoutingAttention after each PMANet_S subnet
        self.net1 = BiLevelRoutingAttention(512)
        self.net2 = BiLevelRoutingAttention(512)
        self.net3 = BiLevelRoutingAttention(512)
        self.net4 = BiLevelRoutingAttention(512)
...
#         print(f"posture shape: {posture.shape}")
...
def main(use_cuda=True, EPOCHS=100, batch_size=48):
    ...
    summary(model, input_size=[
        (batch_size, 48, 224, 224),  # img1
        (batch_size, 48, 224, 224),  # img2
        (batch_size, 48, 224, 224),  # img3
        (batch_size, 48, 224, 224),  # img4
        (batch_size, 48, 64, 64),    # face
        (batch_size, 192, 112, 112), # body
        (batch_size, 3, 16, 26, 1),  # gesture
        (batch_size, 3, 16, 42, 1)   # posture
    ])

    # 定义输入数据
    input_data = (
        torch.randn(batch_size, 48, 224, 224).cuda(),
        torch.randn(batch_size, 48, 224, 224).cuda(),
        torch.randn(batch_size, 48, 224, 224).cuda(),
        torch.randn(batch_size, 48, 224, 224).cuda(),
        torch.randn(batch_size, 48, 64, 64).cuda(),
        torch.randn(batch_size, 192, 112, 112).cuda(),
        torch.randn(batch_size, 3, 16, 26, 1).cuda(),
        torch.randn(batch_size, 3, 16, 42, 1).cuda(),
    )

    # 计算 FLOPs
    print("Calculating FLOPs...")
    flops = FlopCountAnalysis(model, input_data)
    print(f"模型的 FLOPs: {flops.total()}")
    ...
    avg_time_per_inference = (end_time - start_time) / num_iters
...
if mode == "train":
    main(use_cuda=cuda, batch_size=24)
    print("main mode...")
elif mode == "test":
    test(use_cuda=cuda, batch_size=24)
    print("test mode...")
